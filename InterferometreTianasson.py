...
for i in range (0, nF):
        FX = np.linspace(Fx[i] - EFx[i]/2 , Fx[i] + EFx[i]/2 , int(10*EFx[i]/res)+1 )
        FY = np.linspace(Fy[i] - EFy[i]/2 , Fy[i] + EFy[i]/2 , int(10*EFy[i]/res)+1 )
        FZ = np.linspace(Fz[i] - EFz[i]/2 , Fz[i] + EFz[i]/2 , int(10*EFz[i]/res)+1 )
...
plt.pcolormesh(AMPLADA, ALCADA, np.transpose(Val), cmap = cm.gray , shading='gouraud', vmin=0 )
plt.show()